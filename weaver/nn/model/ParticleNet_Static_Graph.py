...
def get_graph_feature_v1(x, k, idx):
    ...
    fts = fts.permute(0, 3, 1, 2).contiguous()  # (batch_size, num_dims, 000000000000000, k)
...
class StaticEdgeConvBlock(nn.Module):
    ...
    def forward(self, features, topk_indices):

        # topk_indices = knn(points, self.k)
        # print("self.k=", self.k)
        x = self.get_graph_feature(features, self.k, topk_indices.long())
...
class ParticleNet(nn.Module):
    ...
    def __init__(self,
        ...
        for idx, layer_param in enumerate(conv_params):
            k, channels = layer_param
            in_feat = input_dims if idx == 0 else conv_params[idx - 1][1][-1]
            self.edge_convs.append(StaticEdgeConvBlock(k=k, in_feat=in_feat, out_feats=channels, cpu_mode=for_inference))
    ...
    def forward(self, features, topk_indices, mask=None):
#         print('points:\n', points)
#         print('features:\n', features)
        start = time.time()
        if mask is None:
        ...
        for idx, conv in enumerate(self.edge_convs):
            ...
            # fts = conv(fts, topk_indices) * mask
            fts = conv(fts, topk_indices)
            if self.use_fusion:
                outputs.append(fts)
        if self.use_fusion: # False
