...
def train_classification(model, loss_func, opt, scheduler, train_loader, dev, epoch, steps_per_epoch=None, grad_scaler=None, tb_helper=None):
    ...
    if("Static" in model.__class__.__name__):
        return train_classification_static(model, loss_func, opt, scheduler, train_loader, dev, epoch, steps_per_epoch, grad_scaler, tb_helper)
    else:
        model.train()
...
def train_classification_static(model, loss_func, opt, scheduler, train_loader, dev, epoch, steps_per_epoch=None, grad_scaler=None, tb_helper=None):
    model.train()
    ...
    with tqdm.tqdm(train_loader) as tq:
        for X, y, _ in tq:
            ...
            label_counter.update(label.cpu().numpy())
            label = label.to(dev)
            top_indice = knn(inputs[0], 16)
            inputs.append(top_indice)
            opt.zero_grad()
            with torch.cuda.amp.autocast(enabled=grad_scaler is not None):
                model_output = model(*inputs)
                logits = _flatten_preds(model_output, label_mask)
...
def evaluate_classification_static(model, test_loader, dev, epoch, for_training=True, loss_func=None, steps_per_epoch=None,
    ...
    with torch.no_grad():
        with tqdm.tqdm(test_loader) as tq:
            for X, y, Z in tq:
                ...
                label_counter.update(label.cpu().numpy())
                label = label.to(dev)
                # sample = {name:X[name] for name in inputs}
                top_indice = knn(inputs[0], 16)
                inputs.append(top_indice)
                # print("input:",inputs)
                aa = [k for k in data_config.input_names]
                # print(aa)
                model_output = model(*inputs)
...
def evaluate_classification(model, test_loader, dev, epoch, for_training=True, loss_func=None, steps_per_epoch=None,
    ...
    print("name=",model.__class__.__name__)
    if("Static" in model.__class__.__name__):
        return evaluate_classification_static(model, test_loader, dev, epoch, for_training, loss_func, steps_per_epoch,
